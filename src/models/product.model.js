...
module.exports = {
    ...
    getSearchedProds: async (name) => {
        const sql_query = `select p.product_id, p.product_name, p.unit_price, p.total_stock, p.product_description, p.category_id, p.product_image, v.organization_name, c.alt_name, locate(?, p.product_name) AS found_index from products p join vendor_table v on p.vendor_id = v.vendor_id join new_category c on p.category_id = c.category_id where p.product_name like ? and p.delisted_id_ref is null`;
    ...
    getAllCount: async() => {
        const sql_query = `select count(product_id) as prod_count from products where delisted_id_ref is null;`
        try {
            return await pquery(sql_query);
        } catch (err) {
            throw err;
    ...
    deleteProd: async(id) => {
        const sql_query = `delete from products where product_id = ?`
        const sql_values = [id];
        try {
            return await pquery(sql_query, sql_values);
        } catch (err) {
    ...
    getImageFolderPath: async(id) => {
        const sql_query = `select product_image from products where product_id = ?`
        const sql_values = [id];
        try {
            return await pquery(sql_query, sql_values);
        } catch (err) {
    ...
    getProdDetailsForVendor: async(vendorid) => {
        const sql_query = `select product_id, product_name, unit_price, total_stock, product_description, category_id, product_image from products where vendor_id = ?;`
